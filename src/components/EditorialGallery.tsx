...
export default function EditorialGallery({ wines }: Props) {
  ...
  useEffect(() => {
    ...
    const updateFocus = () => {
      ...
      if (tintRef.current) {
        tintRef.current.style.background = `radial-gradient(ellipse at 50% 50%, ${hexToRgba(accent, 0.06)} 0%, transparent 60%)`;
      }
      dots.forEach((d, i) => {
        const isActive = i === activeIdx;
        d.style.background = isActive ? (i < wines.length ? wines[i].accent : "#9E825A") : "rgba(158,130,90,0.25)";
        d.style.width = isActive ? "44px" : "22px";
      });
    };
